...
from help_func import format_device_name, format_hazard_records, get_device_label
        ...
        if existing_session and not force_login:
            ...
            return (
                jsonify(
                    {
                        "data": {
                            "name": recent_log.get("name"),
                            "userid": recent_log.get("userid"),
                            "login_time": recent_log.get("login_time"),
                            "loggedin_device": recent_log.get("loggedin_device"),
                            "log_id": log_id,
                        },
...
def force_logout_remote():
    ...
    try:
        ...
        return (
            jsonify({"msg": "User logged out from other device", "status": "success"}),
            200,
        )
...
def check_user_session():
    ...
    try:
        ...
        if not user:
            return (
                jsonify({"msg": "User logged out remotely", "status": "success"}),
                200,
            )
...
def inserted_hazard_circle_list():
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            result = cursor.fetchall()
            indus_circle_list = [
                {"indus_circle": row[0]} for row in result if row[0] is not None
            ]
            return jsonify({"status": "success", "data": indus_circle_list}), 200
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if status == "active":
                ...
                cur.execute(
                    "SELECT allowed_users FROM weatherdata.weather_user_license"
                )
...
def add_new_user():
    ...
    try:
        ...
        if status == "active":
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT COUNT(*) FROM weatherdata.licensed_user_auth WHERE status = 'active'"
                )
                active_count = cur.fetchone()[0]
...
def get_user_license():
    ...
    try:
        with conn.cursor() as cursor:
            query = (
                """SELECT * FROM weatherdata.weather_user_license order by id asc;"""
            )
            cursor.execute(query)
            rows = cursor.fetchall()
            colnames = [desc[0] for desc in cursor.description]
...
def change_password():
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if not row:
                return (
                    jsonify({"status": "Bad Request", "message": "User not found"}),
                    400,
                )
...
def get_log_summary_date_wise():
    ...
    try:
        ...
        condition = (
            f"WHERE login_time >= '{start_date}' AND login_time < DATE '{end_date}' + INTERVAL '1 day' "
...
def save_impacted_circle_to_db():
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "success", "message": "Impacted circles saved to database"}
            ),
            200,
        )
